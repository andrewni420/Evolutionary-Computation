...
(defn parse-card
  "Parses a card of the form \"Ah\" into the form [14 \"Hearts\"]
   -> card = [value suit]"
  [card]
  [(value-from-facecard (str (first card)))
   (suit-from-abbr (str (second card)))])
...
[:betting-round :betting-round :betting-round 0 :integer_eq 3 :integer_eq
 :exec_if :hand-strength 3 :integer_gte
 :exec_if :my-stack {:type :probability :literal 0.1} :equity :all-in
 :total-weight :inc-log :call
 :total-weight :inc-log :bet 'close
 :fold :total-weight :inc-log :call 'close
 'close
 :exec_if :holecard-winrate {:type :probability :literal 0.5} :probability_gte 
     :exec_if :pot
     :check
...
;;hand [hands-that-can-call
;;      win-percent-when-called
;;      S-C Number]
;;Measure robustness - total hands = 1225
;;maybe hands under 0.35 = fragile, hands over 0.4 = robust
