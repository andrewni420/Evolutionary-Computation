 ...
 * shape is (-1, 128, 1024)
 */
public final class EmbedBlock extends AbstractBlock implements Embedding{
    ...
    private EmbedBlock(Builder builder) {
    ...
    protected NDList forwardInternal(
        ...
        NDArray weightArr = ps.getValue(embedding, input.getDevice(), training);
        return embed(input, weightArr);
    }
    ...
    public NDList reverse 
        (ParameterStore ps, NDList inputs, boolean training, PairList<String, Object> params) {
            NDArray input = inputs.singletonOrThrow();
            NDArray weightArr = ps.getValue(embedding, input.getDevice(), training);
            return unembed(input, weightArr);
    }

    public static NDList embed(NDArray input, NDArray weight) {
        return input.getNDArrayInternal().linear(input, weight, null);
    }

    public static NDList unembed(NDArray input, NDArray weight) {
        return input.getNDArrayInternal().linear(input, weight.transpose(), null);
    }
    ...
    /**
     * Creates a builder to build a {@code EmbedBlock}.
     *
    ...
    /** The Builder to construct an {@link EmbedBlock} type of {@link Block}. */
    public static final class Builder {
         ...
         *
         * @return the constructed {@code EmbedBlock}
