...
#_(hands-with-values #{#{6} #{-14 14} #{13 -13} #{-12 12} #{2} #{-10 10} #{5} #{11 -11} #{4}}
                     []
                     45)
  ...
  (let [other-cards (- 52 2 (count community))
        ...
        candidates (for [c1 m  c2 m
                         ...
                         :when (>= (+ (count c1) (count c2) 2)
                                   (+ size1 size2))]
                     [c1 c2])]
    (float (/ (hands-with-values (transduce (map #(get-possible-hands % pattern))
...
(def type-to-pattern
  ...
  {"Four of a Kind" [[4 2] [1 2]]
   "Full House" [[3 2] [2 2]]
   "Three of a Kind" [[3 2] [1 3]]
   "Two Pair" [[2 2] [2 2]]
   "Pair" [[2 2] [1 2]]
   "High Card" [[1 2] [1 2]]})
...
#_(prob-better-hand-rough [[2 "Hearts"] [9 "Spades"] [10 "Clubs"]]
                          [[11 "Hearts"] [11 "Clubs"]])
